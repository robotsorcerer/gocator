...
GoFx(kStatus) GoDiscovery_Init(GoDiscovery discovery, kType type, kAlloc alloc)
{
    ...
    kTry
    {
        ...
        localIpEntry.name[0] = '\0';
        localIpEntry.address = kIpAddress_LoopbackV4();
        kTest(kArrayList_Add(addresses, &localIpEntry));
...
GoFx(kStatus) GoDiscovery_GetAddress(GoDiscovery discovery, k32u deviceId, GoAddressInfo* address)
{
    ...
    do
    {
        if (kSuccess(kUdpClient_Receive(obj->receiver, &remoteEndPoint, kNULL, kTimer_Remaining(obj->stopwatch)))
            ...
            && kSuccess(GoDiscovery_ParseGetAddressReply(discovery, &info))
            && info.id == deviceId)
        {                    
...
GoFx(kStatus) GoDiscovery_GetExtendedInfo(GoDiscovery discovery, k32u deviceId, GoDiscoveryExtInfo* info, kAlloc allocator)
{
    ...
    for (i = 0; i < kArrayList_Count_(obj->interfaces); ++i)
    {
        ...
        kCheck(kSerializer_Write32u(iface->writer, GO_DISCOVERY_GET_INFO_SIZE));
        kCheck(kSerializer_Write32u(iface->writer, 0));
        kCheck(kSerializer_Write32u(iface->writer, GO_DISCOVERY_GET_INFO));
        kCheck(kSerializer_Write32u(iface->writer, 0));
        kCheck(kSerializer_Write64u(iface->writer, GO_DISOVERY_SIGNATURE));
        kCheck(kSerializer_Write32u(iface->writer, deviceId));
        kCheck(kSerializer_Write32u(iface->writer, 0));
    ...
    kCheck(sendStatus);
    kCheck(kTimer_Start(obj->stopwatch, GO_DISCOVERY_GET_INFO_TIMEOUT));
