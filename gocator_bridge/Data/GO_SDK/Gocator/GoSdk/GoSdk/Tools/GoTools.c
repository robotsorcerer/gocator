...
GoFx(const GoMeasurementOption*) GoToolOption_MeasurementOptionAt(GoToolOption option, kSize index)
{
    ...
    return (const GoMeasurementOption*)kArrayList_At(obj->measurementOptions, index);
}
...
GoFx(kStatus) GoTools_Write(GoTools tools, kXml xml, kXmlItem item)
{
    ...
    for (i = 0; i < GoTools_ToolCount(tools); i++)
    {
        ...
        kCheck(GoUtils_FormatToolType(kObject_Type(currentTool), toolName, kCountOf(toolName)));
...
GoFx(kStatus) GoTools_LegacyOptionsRead(GoTools tools, kXml xml, kXmlItem item)
{
    ...
    kTry
    {
        ...
        for (i = 0; i < kArrayList_Count(tempArrayList); i++)
        {
            ...
            if (addOptionToList)
            {
                kCheck(kArrayList_Insert(obj->toolOptions, i, &option));
            }
        }
        ...
        kTry
        {
            for (i = 0; i < kXml_ChildCount(xml, optionsItem); i++)
            {
                    ...
                    || !kStrEquals(GoToolOption_Name(option), kXml_ItemName(xml, toolOptionNode)))
                {
                    ...
                    {
                        kCheck(kArrayList_Resize(opt->measurementOptions, kXml_ChildCount(xml, measurementOptions)));
...
GoFx(kStatus) GoUtils_ParseToolType(const kChar* toolName, kType* type)
{
    ...
    else if (strcmp(toolName, "ProfileBridgeValue") == 0)               *type = kTypeOf(GoProfileBridgeValue);
    ...
    else if (strcmp(toolName, "ProfileXLine") == 0)                     *type = kTypeOf(GoProfileXLine);
...
GoFx(kStatus) GoUtils_FormatToolType(kType type, kChar* toolName, kSize capacity)
{
    const kChar* output = kNULL; 

    if      (type == kTypeOf(GoRangePosition))              output = "RangePosition";
    else if (type == kTypeOf(GoRangeThickness))             output = "RangeThickness";
             
    else if (type == kTypeOf(GoProfileArea))                output = "ProfileArea";
    else if (type == kTypeOf(GoProfileBridgeValue))         output = "ProfileBridgeValue";
    else if (type == kTypeOf(GoProfileBox))                 output = "ProfileBoundingBox";
    else if (type == kTypeOf(GoProfileCircle))              output = "ProfileCircle";
    else if (type == kTypeOf(GoProfileDim))                 output = "ProfileDimension";
    else if (type == kTypeOf(GoProfileGroove))              output = "ProfileGroove";
    else if (type == kTypeOf(GoProfileIntersect))           output = "ProfileIntersect";
    else if (type == kTypeOf(GoProfileLine))                output = "ProfileLine";
    else if (type == kTypeOf(GoProfilePanel))               output = "ProfilePanel";
    else if (type == kTypeOf(GoProfilePosition))            output = "ProfilePosition";
    else if (type == kTypeOf(GoProfileStrip))               output = "ProfileStrip";
    else if (type == kTypeOf(GoProfileXLine))               output = "ProfileXLine";
             
    else if (type == kTypeOf(GoSurfaceBox))                 output = "SurfaceBoundingBox";
    else if (type == kTypeOf(GoSurfaceCountersunkHole))     output = "SurfaceCsHole";
    else if (type == kTypeOf(GoSurfaceEllipse))             output = "SurfaceEllipse";
    else if (type == kTypeOf(GoSurfaceHole))                output = "SurfaceHole";
    else if (type == kTypeOf(GoSurfaceOpening))             output = "SurfaceOpening";
    else if (type == kTypeOf(GoSurfacePlane))               output = "SurfacePlane";
    else if (type == kTypeOf(GoSurfacePosition))            output = "SurfacePosition";
    else if (type == kTypeOf(GoSurfaceStud))                output = "SurfaceStud";
    else if (type == kTypeOf(GoSurfaceVolume))              output = "SurfaceVolume";
             
    else if (type == kTypeOf(GoScript))                     output = "Script";
    else if (type == kTypeOf(GoExtTool))                    output = "Custom";
...
GoFx(const kChar*) GoTools_BuiltInToolDefineToString(GoToolType type)
{
    ...
    case GO_TOOL_PROFILE_BRIDGE_VALUE: return "ProfileBridgeValue"; 
    ...
    case GO_TOOL_PROFILE_X_LINE: return "ProfileXLine";           
...
GoFx(GoToolType) GoTools_StringToBuiltInDefine(const kChar* name)
{
    ...
    if (strcmp(name, "ProfileBridgeValue") == 0) return GO_TOOL_PROFILE_BRIDGE_VALUE;             
    ...
    if (strcmp(name, "ProfileXLine") == 0) return  GO_TOOL_PROFILE_X_LINE;            
...
GoFx(kBool) GoTools_IsToolInOptionsList(GoTools tools, kType toolType)
{
    ...
    kCheck(GoUtils_FormatToolType(toolType, toolName, kCountOf(toolName)));
...
GoFx(const GoMeasurementOption*) GoTools_FindMeasurementOption(GoTools tools, kType toolType, kType measurementType)
{
    ...
    if (kSuccess(GoUtils_FormatToolType(toolType, toolName, kCountOf(toolName)))
