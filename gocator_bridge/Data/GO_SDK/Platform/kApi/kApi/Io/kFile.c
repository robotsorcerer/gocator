...
kFx(kStatus) kFile_Init(kFile file, kType type, const kChar* path, kFileMode mode, kAlloc allocator)
{
    ...
    {         
        ...
        kTest(kApiLib_FileHandlers_()->seek(file, 0, kSEEK_ORIGIN_END, &obj->streamLength)); 

        kTest(kApiLib_FileHandlers_()->seek(file, 0, kSEEK_ORIGIN_BEGIN, &zero));  
    }
    kCatch(&status) 
    {
        kFile_VRelease(file); 
...
kFx(kStatus) kFile_VSeek(kFile file, k64s offset, kSeekOrigin origin)
{
    ...
    kCheck(kApiLib_FileHandlers_()->seek(file, offset, origin, &obj->streamPosition)); 
...
kFx(kStatus) kFile_SeekImpl(kFile file, k64s offset, kSeekOrigin origin, k64u* position)
{
    kFileClass *obj = kFile_Cast_(file); 
    DWORD moveMethod; 
...
kFx(kStatus) kFile_OpenImpl(kFile file, const kChar* path, kFileMode mode)
{
    ...
    if ((obj->handle = open(nativePath, oflag, S_IRWXU)) == -1)
    {
        switch (errno)
        {
        case ENOENT:
