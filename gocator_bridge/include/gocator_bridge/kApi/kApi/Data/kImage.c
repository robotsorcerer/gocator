...
kFx(kStatus) kImage_VInitClone(kImage image, kImage source, kAlloc allocator)
{
    ...
    kTry
    {
        kTest(kImage_Assign(image, source));  
    }
    kCatch(&status)
    {
        kImage_VRelease(image); 
...
kFx(kStatus) kImage_CopyPixels(kImage image, kImage source)
{
    kImageClass* obj = kImage_Cast_(image); 
    kImageClass* srcObj = kImage_Cast_(source); 
    kSize i; 

    kCheckArgs((obj->width == srcObj->width) && (obj->height == srcObj->height) && (obj->pixelSize == srcObj->pixelSize)); 

    if (obj->stride == srcObj->stride)
    {
        kCheck(kMemCopy(obj->pixels, srcObj->pixels, obj->stride*obj->height));
    }
    else
    {
        for (i = 0; i < obj->height; ++i)
        {
            ...
            kCheck(kMemCopy(row, srcRow, obj->pixelSize*obj->width)); 
        }
    }
...
kFx(kStatus) kImage_Allocate(kImage image, kType pixelType, kSize width, kSize height)
{
    ...
    kSize newPixelSize = kType_Size_(newPixelType); 
    kSize newStride = kAlign_(newWidth * newPixelSize, kIMAGE_ALIGNMENT); 
...
kFx(kStatus) kImage_Assign(kImage image, kImage source)
{
    ...
    kCheck(kImage_CopyPixels(image, source)); 
...
kFx(kStatus) kImage_Zero(kImage image)
{
    ...
    return kMemSet(obj->pixels, 0, kImage_DataSize_(image)); 
}
