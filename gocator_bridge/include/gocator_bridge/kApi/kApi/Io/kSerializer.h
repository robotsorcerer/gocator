 ...
 * 
 * By default, kSerializer assumes that primative values should be encoded to (or decoded from) little endian format. Accordingly, on 
...
kStatus ReadData(kStream stream, MyData* data)
{
    ...
    kTry
    {
        //construct a plain serializer object (can write/write primative types and arrays)
        ...
        kTestErr(data->valueCount <= kCountOf(data->values), FORMAT); 
 ...
 * @return                  Operation status. 
 */
kFx(kStatus) kSerializer_SetEndianness(kSerializer serializer, kEndianness endianness); 
