...
kFx(kStatus) kSocket_BeginConnect(kSocket sockt, kIpAddress address, k32u port)
{
    ...
    kTry
    {
        ...
        obj->beginConnectLastError = kSocket_GetLastError(); 
    }
    kCatch(&exception)
    {
...
kFx(kStatus) kSocket_EndConnect(kSocket sockt, k64u timeout)
{
    ...
    {     
        ...
        if (obj->beginConnectResult == kSOCKET_ERROR)
        {
            if (obj->beginConnectLastError == kSOCKET_WOULD_BLOCK || obj->beginConnectLastError == kSOCKET_IN_PROGRESS)
            {                
                FD_ZERO(&writeSet); 
                FD_ZERO(&exceptSet); 
...
kFx(kPointer) kSocket_Handle(kSocket sockt)
{
    kSocketClass* obj = kSocket_Cast_(sockt); 

    return (kPointer) (kSize) obj->handle; 
}
