...
kBeginClass(k, kTcpClient, kStream)
    ...
    kAddVMethod(kTcpClient, kStream, VSeek)
kEndClass()
...
//support forward seek by reading and discarding; occasionally useful to unify client code for files/sockets
kFx(kStatus) kTcpClient_VSeek(kTcpClient client, k64s offset, kSeekOrigin origin)
{
    kByte buffer[256]; 
    k64s remaining = offset; 

    switch (origin)
    {
    case kSEEK_ORIGIN_BEGIN: 
        remaining -= kStream_BytesRead_(client); 
        break;
    ...
    kCheckArgs(remaining >= 0); 

    while (remaining > 0)
    {
        k64u chunkSize = kMin_((k64u)remaining, sizeof(buffer)); 
