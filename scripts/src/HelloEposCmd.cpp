...
int DemoProfilePositionMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	...
	else
	{
		...
		positionList.push_back(1000);
		positionList.push_back(-300);
		positionList.push_back(-600);
...
bool DemoReverseVelocityMode(HANDLE p_DeviceHandle, unsigned short p_usNodeId, unsigned int & p_rlErrorCode)
{
	int lResult = MMC_SUCCESS;
	stringstream msg;
...
/*	else
	...
	lResult = DemoReverseVelocityMode(g_pKeyHandle, g_usNodeId, lErrorCode);

	if(lResult != MMC_SUCCESS)
	{
		LogError("DemoReverseVelocityMode", lResult, lErrorCode);
	}
...
{
	// ros::init(argc, argv, "Epos_controller");
	int lResult = MMC_FAILED;
